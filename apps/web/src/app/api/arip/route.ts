  ...
  try {
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        aip_issued_date: record.aipIssuedDate?.toISOString().slice(0, 10) ?? null,
        aip_expiry_date: record.aipExpiryDate?.toISOString().slice(0, 10) ?? null,
        aip_days_remaining: aipDaysRemaining,
        // Customer cap tracking (absolute counts — Section 29, ARIP Framework)
        ...
        solicitor_name: record.solicitorName ?? null,
        solicitor_firm: record.solicitorFirm ?? null,
        solicitor_engaged_date: record.solicitorEngagedDate?.toISOString().slice(0, 10) ?? null,
        fidelity_bond_in_place: record.fidelityBondInPlace,
        fidelity_bond_expiry: record.fidelityBondExpiry?.toISOString().slice(0, 10) ?? null,
        application_fee_paid: record.applicationFeePaid,
        application_fee_amount_ngn: record.applicationFeeAmountNgn ?? null,
        sec_reference_number: record.secReferenceNumber ?? null,
        // Legacy fields (kept for existing dashboard components)
        arip_entry_customer_count: extra.arip_entry_customer_count ?? null,
        current_customer_count: extra.current_customer_count ?? record.aipTotalCustomers,
        growth_cap_breached: extra.growth_cap_breached ?? (record.aipTotalCustomers >= record.aipMaxCustomers),
